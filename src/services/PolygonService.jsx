...
class PolygonService {
  ...
  async getAggregatesWithExtendedHours(ticker, from, to, multiplier = 1, timespan = 'minute') {
    try {
      ...
      console.error(`[PolygonService] Failed to get ${multiplier}${timespan} aggregates:`, error);
      return [];
    }
  }
  ...
  async getPreviousDayClose(ticker, daysBack = 1) {
    try {
      ...
      console.warn(`[PolygonService] Could not get previous close for ${ticker}:`, error.message);
  ...
  async getSingleTickerSnapshot(ticker) {
    try {
      ...
      console.error(`[PolygonService] Failed to get snapshot for ${ticker}:`, error);
      return null;
    }
  }
